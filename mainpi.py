...
def process_frame(frame, chalkboard_coords, chalkboard_corners, chalkboard_confirmed, start_time, last_detection_time, average_color, writing_boxes, last_writing_check, last_area, ignored_oranges, chalkboard_roi):
    ...
    if chalkboard_confirmed:
        ...
        for gx, gy, gw, gh in merged_box:
            ...
            if DEBUG_VIEW:
                # Save the frame with detected writing for debugging
                debug_filename = f"debug_frame_{time.time()}.png"
        ...
        if chalkboard_roi is not None:
            ...
            if orange_x is not None and orange_y is not None:
                if all(np.linalg.norm(np.array([orange_x, orange_y]) - np.array([ox, oy])) > ORANGE_THRESHOLD for ox, oy in ignored_oranges):
                    ...
                    if DEBUG_VIEW:
                        # Save the frame with detected orange circle for debugging
                        debug_filename = f"debug_orange_{time.time()}.png"
                        cv2.imwrite(debug_filename, frame)
                        print(f"Saved debug frame: {debug_filename}")
                    ...
                    if orange_count > 0:
                        ...
                        if isinstance(math_result, str):
                            ...
                            # Save the generated plot for debugging
                            plot_filename = f"generated_plot_{time.time()}.png"
                            cv2.imwrite(plot_filename, plot_image)
                            print(f"Saved generated plot: {plot_filename}")
                    else:
                        orange_count += 1
...
def main():
    ...
    while True:
        ...
        if DEBUG_VIEW:
            # Save the frame with detected chalkboard for debugging
            debug_filename = f"debug_chalkboard_{time.time()}.png"
            cv2.imwrite(debug_filename, frame)
            print(f"Saved debug frame: {debug_filename}")
