...
AREA_THRESHOLD = 1000
DISTANCE_THRESHOLD = 1000  # Distance threshold to group writing areas
...
def detect_writing(frame, chalkboard, average_color):
    ...
    diff = cv2.absdiff(cv2.cvtColor(chalkboard_roi, cv2.COLOR_BGR2GRAY), np.full_like(gray_roi, int(np.mean(average_color))))
...
def send_to_text_detection_service(image):
    ...
    if response.status_code == 200:
        ...
        print("Error in text detection service:", response.text)
        return []

def overlap(box1, box2):
    return not (box1[0] + box1[2] < box2[0] or
                box1[0] > box2[0] + box2[2] or
...
while True:
    ...
    if chalkboard_confirmed:
        ...
        if current_time - last_writing_check > WRITING_CHECK_INTERVAL:
            ...
            if detected_boxes:
                writing_boxes = [(int(box['startX']), int(box['startY']), int(box['endX']) - int(box['startX']), int(box['endY']) - int(box['startY'])) for box in detected_boxes]
            else:
                writing_boxes = []
        ...
        merged_box = merge_boxes(grouped_boxes, margin=10, x_max=w, y_max=h)
